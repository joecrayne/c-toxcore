...
typedef struct {
    ...
    void (*audio_data)(Messenger *m, uint32_t group_number, uint32_t peernumber, const int16_t *pcm, uint32_t samples,
                               ...
                               uint32_t friendgroup_number)
{
    ...
    if (out_audio) {
        ...
        if (group_av->audio_data) {
            group_av->audio_data(group_av->g_c->m, group_number, friendgroup_number, out_audio, out_audio_samples,
 ...
 * return -1 on failure.
 */
static int groupchat_enable_av(const Logger *log, Group_Chats *g_c, uint32_t group_number,
