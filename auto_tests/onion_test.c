...
START_TEST(test_basic)
{
    ...
    Onion_Announce *onion1_a = new_onion_announce(onion1->dht, new_gca_list());
    Onion_Announce *onion2_a = new_onion_announce(onion2->dht, new_gca_list());
...
static Onions *new_onions(uint16_t port, uint32_t *index)
{
    ...
    on->onion_a = new_onion_announce(dht, new_gca_list());
    ...
    on->onion_c = new_onion_client(new_net_crypto(on->log, dht, &inf), nullptr);
