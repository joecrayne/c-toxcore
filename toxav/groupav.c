...
static void group_av_peer_new(void *object, uint32_t group_number, uint32_t friendgroup_number)
{
    ...
    if (group_peer_set_object(group_av->g_c, group_number, friendgroup_number, peer_av) == -1) {
        free(peer_av);
    }
}

static void group_av_peer_delete(void *object, uint32_t group_number, void *peer_object)
{
                               ...
                               uint32_t friendgroup_number)
{
    ...
    if (out_audio) {
        ...
        if (group_av->audio_data) {
            group_av->audio_data(group_av->tox, group_number, friendgroup_number, out_audio, out_audio_samples,
                                     ...
                                     const uint8_t *packet, uint16_t length)
{
    ...
    while (decode_audio_packet((Group_AV *)object, peer_av, group_number, friendgroup_number) == 0) {
        continue;
    }
 ...
 * return -1 on failure.
 */
static int groupchat_enable_av(const Logger *log, Tox *tox, Group_Chats *g_c, uint32_t group_number,
                               audio_data_cb *audio_callback, void *userdata)
{
    ...
    if (group_set_object(g_c, group_number, group_av) == -1
            || callback_groupchat_peer_new(g_c, group_number, group_av_peer_new) == -1
            || callback_groupchat_peer_delete(g_c, group_number, group_av_peer_delete) == -1
            || callback_groupchat_delete(g_c, group_number, group_av_groupchat_delete) == -1) {
...
int add_av_groupchat(const Logger *log, Tox *tox, Group_Chats *g_c, audio_data_cb *audio_callback, void *userdata)
{
    ...
    if (groupchat_enable_av(log, tox, g_c, group_number, audio_callback, userdata) == -1) {
...
static int send_audio_packet(Group_Chats *g_c, uint32_t group_number, uint8_t *packet, uint16_t length)
{
    ...
    *ptr = GROUP_AUDIO_PACKET_ID;
    ...
    if (send_group_lossy_packet(g_c, group_number, data, plen) == -1) {
        return -1;
    }
 ...
 * return -1 on failure.
 */
int group_send_audio(Group_Chats *g_c, uint32_t group_number, const int16_t *pcm, unsigned int samples, uint8_t channels,
                     uint32_t sample_rate)
{
    ...
    return send_audio_packet(g_c, group_number, encoded, size);
}
